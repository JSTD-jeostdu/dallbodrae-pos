...
# Phase 1.5 — 매니저 역할 (칸반 보드)

> 2026-09-21 선생님 인터뷰로 추가한 태스크다. 주문·제조·호출을 한 화면에서 보고 관리한다.
> Phase 2의 Task 14(주문 취소) 중 **취소 자체**를 앞당긴다. 재료 복구는 재고 기능이 생기는
> Task 13~14에서 이 태스크의 `cancel` 전이에 붙인다.

### Task 11.5: 매니저 역할 — 수령 단계 · 취소 · 다시 호출 · 칸반 보드
...
|---|---|
| 사용자 | 선생님 전용. 비밀번호 없음 |
| 보드 열 | 제조 중 → 준비 완료 → 수령 완료, 그리고 취소 (4열) |
| 조작 | 카드의 버튼 (끌어다 놓기 없음) |
| 동작 | 단계 옮기기 · 다시 호출 · 주문 취소 |
| 카드 정보 | 주문번호(크게) + 메뉴와 수량 |
| 화면 모드 | 태블릿 모드(4열 나란히) / 모바일 모드(열 하나씩) — 고를 수 있음 |
| 수령 처리 | 매니저가 누르고, 제조 화면에서도 `가져감`으로 누를 수 있음 |
| 호출 화면 준비 완료 | 수령·취소된 번호는 바로 뺀다. 10개 제한은 그대로 |
| 호출 화면 가운데 | 마지막으로 부른 번호가 수령·취소되면 비우고 안내 문구를 띄운다 |
| 다시 호출 | 처음 호출과 똑같이 (가운데 크게 · 차임음 · 음성 · 목록 맨 끝으로) |
| 취소 범위 | 제조 중 · 준비 완료. 확인창 + 되돌리기 |
| 수령·취소 열 | 오늘 것 전부, 최신이 위, 열 제목에 건수 |

**선생님 확인 없이 정한 것** — 바꾸려면 여기부터 고친다

1. 다시 호출은 `readyAt`을 바꾸지 않고 `calledAt`을 따로 둔다. `readyAt`은 Phase 2 판매 기록에서 제조 시간 계산에 쓴다.
2. `수령 ▶`·`가져감`도 잠깐 `되돌리기`를 띄운다.
3. 모드는 기기마다 기억한다. 처음에는 화면 폭(700px)으로 고른다.
4. 제조 중·준비 완료 열은 날짜와 상관없이 끝나지 않은 주문을 모두 보여준다.
5. 취소는 문서를 지우지 않고 `status: 'cancelled'`와 원래 단계(`cancelledFrom`)를 남긴다.

**데이터**

```
order.status        'waiting' | 'ready' | 'pickedUp' | 'cancelled'
order.readyAt       완료한 시각 (다시 호출로 바뀌지 않는다)
order.calledAt      마지막으로 부른 시각 (완료할 때와 다시 호출할 때 적힌다) — 없으면 readyAt
order.pickedUpAt    손님이 가져간 시각
order.cancelledAt   취소한 시각
order.cancelledFrom 취소 직전 단계 ('waiting' | 'ready') — 되살릴 때 돌아갈 곳
```

Firestore 규칙은 바꾸지 않는다 (모두 `orders` 컬렉션 안의 필드다).

**Interfaces**
- Produces (순수):
  - `callTime(order) → number` — `calledAt`, 없으면 `readyAt`
  - `orderPatch(action, order, now) → patch` — `complete` · `backToMaking` · `recall` · `pickUp` · `unPickUp` · `cancel` · `restore`
  - `displayBoard(orders, readyMax, waitingMax)` — `current`가 "마지막으로 부른 주문이 아직 준비 완료일 때만" 채워지도록 바꾼다
  - `managerBoard(orders, businessDate) → { waiting, ready, pickedUp, cancelled }`
- Produces (화면): `VIEWS.manager`, 제조 화면의 `준비 완료` 구역
- Changes: `recentCalls`와 새 호출 감지가 `readyAt` 대신 `callTime`을 쓴다

- [ ] **Step 1: 상태 전이와 보드 계산 테스트 → 구현**

| 테스트 | 기대 |
|---|---|
| `complete` | `status: 'ready'`, `readyAt`·`calledAt` = now |
| `backToMaking` | `status: 'waiting'`, `readyAt`·`calledAt` = null |
| `recall` | `calledAt`만 now. `readyAt`은 그대로 |
| `pickUp` / `unPickUp` | `pickedUp` + `pickedUpAt` / `ready` + `pickedUpAt: null` |
| `cancel` | `cancelled` + `cancelledAt` + `cancelledFrom`(직전 상태) |
| `restore` | `cancelledFrom`으로 돌아간다. 없으면 `waiting` |
| 다시 호출한 번호 | 준비 완료 목록 맨 끝으로 가고 `current`가 된다 |
| 마지막으로 부른 번호가 수령됨 | `current`는 null, 준비 완료 목록에서도 빠진다 |
| 취소된 번호 | 준비 완료 목록에서 빠진다 |
| `managerBoard` | 수령·취소는 오늘 것만, 최신이 위. 제조 중은 오래된 것부터, 준비 완료는 먼저 부른 것부터 |

- [ ] **Step 2: 기존 화면 반영**
  - 제조 화면 `완료` → `orderPatch('complete')`, 되돌리기 → `backToMaking`
  - 제조 화면 아래에 `준비 완료` 카드와 `가져감` 버튼 (+ 되돌리기)
  - 호출 화면: 가운데가 비면 "음료가 준비되면 번호를 불러 드려요."
  - 새 호출 감지를 `callTime` 기준으로 — 다시 호출은 울리고, 되돌리기·수령은 울리지 않는다

- [ ] **Step 3: 매니저 보드 — 태블릿 모드**
  - 역할 선택에 `📋 매니저 (선생님)` 추가, 상단바 표시
  - 4열 나란히, 열마다 따로 스크롤. 다시 그려도 스크롤 위치를 지킨다
  - 열별 버튼: 제조 중 `완료 ▶`·`취소` / 준비 완료 `◀ 제조로`·`다시 호출`·`수령 ▶`·`취소` / 수령 완료 `◀ 준비 완료로` / 취소 `되살리기`

- [ ] **Step 4: 모바일 모드**
  - 위에 4열 탭(건수 포함), 고른 열 하나만 보여준다
  - `태블릿 | 모바일` 전환 버튼, 기기마다 기억

- [ ] **Step 5: 화면 확인**

| 확인할 것 | 기대 |
|---|---|
| 매니저 `완료 ▶` | 호출 화면에 차임음·음성·번호 |
| `다시 호출` | 같은 번호가 다시 크게 · 음성 · 목록 맨 끝 |
| `수령 ▶` (가운데 번호) | 호출 화면 가운데가 비고 목록에서 빠짐. 소리 없음 |
| 제조 화면 `가져감` | 매니저 보드에서 수령 완료로 이동 |
| `취소` | 확인창 → 취소 열로. 되돌리기 가능 |
| `되살리기` | 원래 단계로 돌아감 |
| 모바일 모드 | 탭으로 열 전환, 가로 넘침 없음 |
| 다시 그리기 | 수령 완료 열을 내려 둔 채 새 주문이 들어와도 스크롤이 튀지 않음 |

- [ ] **Step 6: 사용법 화면 · README에 매니저 역할 추가, 커밋, 배포**

---

# Phase 2 — P1: 재료 재고와 기록

> **2026-09-21 선생님 결정 (Task 11.5 이후 맞춤)**
> - 재료는 **주문할 때** 차감한다. 차감한 양은 주문 문서의 `stockDelta`에 함께 저장한다.
> - 취소는 **매니저에서만** 한다. Task 14의 "주문 화면 최근 주문 목록 · 제조 화면 취소"는 하지 않는다.
> - 취소하면 `stockDelta`를 되돌리고, 되살리면(취소 되돌리기 포함) 다시 차감한다.
>   현재 레시피로 다시 계산하지 않으므로 주문 뒤에 레시피를 고쳐도 복구량이 어긋나지 않는다.
> - 판매로 세는 주문: 제조 중 · 준비 완료 · 수령 완료. 취소만 뺀다.
> - 리허설 데이터를 비우는 기능(원래 Task 19의 데이터 초기화)을 Phase 2 끝에 앞당긴다.
...
> 취소 자체(상태 전이 · 매니저 보드의 버튼)는 Task 11.5에서 이미 만들었다. 여기서는 `cancel` · `restore` 전이에 재료 복구 · 재차감만 붙인다.
